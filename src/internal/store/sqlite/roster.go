...
func (s *Store) ListRosterEntries(ctx context.Context, filter store.RosterFilter) ([]store.RosterEntry, error) {
	...
	for rows.Next() {
		var e store.RosterEntry
		if err := scanSqliteRosterEntry(rows, &e); err != nil {
...
func (s *Store) CreateToken(ctx context.Context, token store.AgentToken) error {
	...
	_, err := s.db.ExecContext(ctx, `
		...
		nullTimePtr(token.ExpiresAt), nullTimePtr(token.RevokedAt),
		nullTimePtr(token.LastUsedAt))
	if err != nil {
		return classifySqliteConflict("CreateToken", err)
	}
	return nil
}
...
func (s *Store) RevokeTokenCascade(ctx context.Context, rosterID string) (int64, error) {
	...
	for len(ids) > 0 {
		...
		rows, err := s.db.QueryContext(ctx,
			`SELECT roster_id FROM agent_roster WHERE parent_ref IN (`+inClause+`)`,
			childArgs...)
		if err != nil {
...
type sqliteRosterScanner interface {
	Scan(dest ...any) error
}

func scanSqliteRosterEntry(row sqliteRosterScanner, e *store.RosterEntry) error {
	var sessionID, parentRef sql.NullString
	var boundAt sql.NullTime
	err := row.Scan(
...
func classifySqliteConflict(op string, err error) error {
	if err == nil {
		return nil
	}
	errStr := err.Error()

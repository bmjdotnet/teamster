// Outcome/WorkUnit/EntityDependency CRUD (wms.Reader/Writer v2 methods) —
// mirrors internal/store/mysql/store_v2.go almost verbatim. The only dialect
// change needed is INSERT IGNORE -> INSERT OR IGNORE; the WITH RECURSIVE
// cycle-detection CTEs are supported identically by SQLite.
package sqlite
...
func (s *Store) ListOutcomes(ctx context.Context, parentOutcomeID string, tagFilters map[string]string, statusFilter string, query string) ([]*wms.Outcome, error) {
	...
	if query != "" {
		...
		sb.WriteString(` AND (o.title LIKE ? ESCAPE '\' OR o.description LIKE ? ESCAPE '\')`)
...
// AddOutcomeEdge: cycle detection via WITH RECURSIVE (identical to MySQL);
// INSERT IGNORE -> INSERT OR IGNORE is the only dialect change.
func (s *Store) AddOutcomeEdge(ctx context.Context, parentID, childID string) error {
	...
	_, err := s.db.ExecContext(ctx,
		`INSERT OR IGNORE INTO outcome_edges (parent_id, child_id) VALUES (?, ?)`, parentID, childID)
	return err
}
...
func (s *Store) AddEntityDependency(ctx context.Context, dep *wms.Dependency) error {
	...
	_, err := s.db.ExecContext(ctx, `
		INSERT OR IGNORE INTO entity_dependencies (blocker_type, blocker_id, blocked_type, blocked_id, created_at)
		VALUES (?, ?, ?, ?, ?)`,

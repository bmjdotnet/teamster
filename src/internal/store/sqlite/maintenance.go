...
var _ store.MaintenanceStore = (*Store)(nil)
...
// non-nil, its ended_at/duration_ms) via a single UPDATE attempt. The
// retry-on-conflict loop stays in the caller, which nudges endedAt and
// retries on ErrConflict. Mirrors the mysql backend: classifyConflict is
// only applied on the endedAt-touching branch — the session_id/agent_name-only
// UPDATE cannot collide with uq_open (entity_type, entity_id, kind, ended_at),
// since it touches none of those columns.
...
//
// MySQL's TIMESTAMPDIFF(MICROSECOND, a, b) / 1000 (whole milliseconds
// between two instants) becomes SQLite's
// CAST((julianday(b) - julianday(a)) * 86400000 AS INTEGER) — julianday
// yields fractional days, and a day has 86400000 milliseconds.
func (s *Store) RepairInterval(ctx context.Context, id int64, newStart, newEnd time.Time, mode string) error {
	...
	if newEnd.IsZero() {
		...
		res, err = tx.ExecContext(ctx, `
			UPDATE wms_intervals
			SET ended_at = ?, duration_ms = CAST((julianday(?) - julianday(?)) * 86400000 AS INTEGER)
			WHERE id = ? AND kind = ? AND started_at = ? AND ended_at IS NOT NULL AND ended_at < started_at`,
			newEnd, newEnd, newStart, id, kind, newStart)
	}
	if err != nil {
	...
	if recordEvidence {
		...
		if _, err := tx.ExecContext(ctx, `
			...
			ON CONFLICT(interval_id) DO UPDATE SET
				prior_ended_at = excluded.prior_ended_at,
				new_ended_at   = excluded.new_ended_at,
...
func (s *Store) UnrepairIntervals(ctx context.Context) (int64, error) {
	...
	for _, rp := range reps {
		if rp.prior.Valid {
			if _, err := tx.ExecContext(ctx, `
				UPDATE wms_intervals
				SET ended_at = ?, duration_ms = CAST((julianday(?) - julianday(started_at)) * 86400000 AS INTEGER)

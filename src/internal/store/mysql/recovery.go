...
// added as a seventh strategy value rather than a separate method, since it
// shares ApplyRecovery/UncoverRecovery's exact skeleton.
...
// strategyEvidenceTable maps a strategy to its evidence table.
...
// passes a prior LLM-sweep skip can be superseded by (see
// recover_directive.go's original doc comment).
...
func evTime(ev map[string]any, key string) time.Time {
...
// + INSERT-evidence transaction, strategy-tagged. Replaces the six
// near-identical applyRecovery/applyWarmupRecovery/applyGapRecovery/
// applyDirective/applySynthesis/applyRemoteFloor bodies. interval_id is
// re-resolved per message via a correlated subquery (the same
// most-recently-started covering-interval rule as StateIntervalAt) rather
// than requiring the caller to resolve it per message first, so the whole
// batch is one set-based UPDATE.
func (s *Store) ApplyRecovery(ctx context.Context, batch store.RecoveryBatch) error {
	...
	now := time.Now().UTC()
	msgPlaceholders := placeholderList(len(batch.MessageIDs))
	methodPlaceholders := placeholderList(len(sourceMethods))
	...
	if batch.Strategy == "skip" {
		...
		intervalExpr := `COALESCE((
			        ...
			        WHERE wi.kind = 'state' AND wi.entity_type = ? AND wi.entity_id = ?
		...
		updateQ = fmt.Sprintf(`
			...
			SET ua.entity_type = ?, ua.entity_id = ?, ua.method = ?, ua.computed_at = ?,
			    ua.interval_id = %s
			WHERE ua.message_id IN (%s) AND ua.method IN (%s)`, intervalExpr, msgPlaceholders, methodPlaceholders)
	...
	if batch.Strategy == "skip" {
		confirmArgs := append([]any{}, anySlice(batch.MessageIDs)...)
		...
		confirmed, err = scanStrings(rows)
		if err != nil {
			return err
		}
	} else {
...
// batch.Evidence for the strategy-specific columns. Each table's columns
// mirror its pre-port schema exactly (migrations v33-v39, v_directive).
func writeEvidence(ctx context.Context, tx *sql.Tx, evidenceTable string, batch store.RecoveryBatch, msgID string, now time.Time) error {
	...
	case "recovery_evidence":
		_, err := tx.ExecContext(ctx, `
			...
			ON DUPLICATE KEY UPDATE
				...
				setfocus_at = VALUES(setfocus_at), recovered_at = VALUES(recovered_at)`,
			msgID, batch.Entity.EntityType, batch.Entity.EntityID, evTime(ev, "setfocus_at"), now)
		return err
	case "warmup_evidence":
		_, err := tx.ExecContext(ctx, `
			...
			ON DUPLICATE KEY UPDATE
				entity_type = VALUES(entity_type), entity_id = VALUES(entity_id),
				warmup_start = VALUES(warmup_start), first_focus_at = VALUES(first_focus_at),
				recovered_at = VALUES(recovered_at)`,
			msgID, batch.Entity.EntityType, batch.Entity.EntityID,
			evTime(ev, "warmup_start"), evTime(ev, "first_focus_at"), now)
		return err
	case "gap_evidence":
		_, err := tx.ExecContext(ctx, `
			...
			ON DUPLICATE KEY UPDATE
				entity_type = VALUES(entity_type), entity_id = VALUES(entity_id),
				session_id = VALUES(session_id), agent_name = VALUES(agent_name),
				resolution_method = VALUES(resolution_method), resolved_from_entity = VALUES(resolved_from_entity),
				recovered_at = VALUES(recovered_at)`,
			msgID, batch.Entity.EntityType, batch.Entity.EntityID,
			evStr(ev, "session_id"), evStr(ev, "agent_name"),
			evStr(ev, "resolution_method"), evStr(ev, "resolved_from_entity"), now)
		return err
	case "directive_evidence":
		_, err := tx.ExecContext(ctx, `
			...
			ON DUPLICATE KEY UPDATE
				entity_type = VALUES(entity_type), entity_id = VALUES(entity_id),
				session_id = VALUES(session_id), agent_name = VALUES(agent_name),
				directive_type = VALUES(directive_type), directive_id = VALUES(directive_id),
				recovered_at = VALUES(recovered_at)`,
			msgID, batch.Entity.EntityType, batch.Entity.EntityID,
			evStr(ev, "session_id"), evStr(ev, "agent_name"),
	...
	case "synthesis_evidence":
		...
		_, err := tx.ExecContext(ctx, `
			...
			ON DUPLICATE KEY UPDATE
				entity_type = VALUES(entity_type), entity_id = VALUES(entity_id),
				session_id = VALUES(session_id), confidence = VALUES(confidence),
				evidence_excerpt = VALUES(evidence_excerpt), mapping_source = VALUES(mapping_source),
				recovered_at = VALUES(recovered_at)`,
			msgID, entityType, entityID, evStr(ev, "session_id"),
			evStr(ev, "confidence"), evStr(ev, "evidence_excerpt"), evStr(ev, "mapping_source"), now)
		return err
	default:
...
func (s *Store) UncoverRecovery(ctx context.Context, strategy string) (int64, error) {
	...
	if _, err := tx.ExecContext(ctx, fmt.Sprintf(
		`DELETE ev FROM %s ev JOIN usage_attribution ua ON ua.message_id = ev.message_id WHERE ua.method = ?`,
...
func (s *Store) ReleaseSessionAttribution(ctx context.Context, sessionID string, methods []string) (int64, error) {
	...
	q := fmt.Sprintf(`
		...
		JOIN token_ledger t ON t.message_id = ua.message_id
		WHERE t.session_id = ? AND ua.method IN (%s)`, placeholderList(len(methods)))
	args := append([]any{sessionID}, anySlice(methods)...)
...
// UnallocatedSessions implements store.RecoveryStore.
func (s *Store) UnallocatedSessions(ctx context.Context, f store.UnallocatedFilter) ([]store.SessionCost, error) {
	...
	if len(f.ExcludeMethods) > 0 {
		q += fmt.Sprintf(` AND t.session_id NOT IN (
			...
			WHERE ua2.method IN (%s))`, placeholderList(len(f.ExcludeMethods)))
...
// reclaimableSessionMessages (F4) — the caller passes the exact method set
// each strategy is allowed to reclaim.
func (s *Store) ReclaimableMessages(ctx context.Context, sessionID, agentName string, agentExact bool, methods []string) ([]store.LedgerMessage, error) {
	q := fmt.Sprintf(`
		...
		FROM usage_attribution ua
		JOIN token_ledger t ON t.message_id = ua.message_id
		WHERE ua.method IN (%s) AND t.session_id = ?`, placeholderList(len(methods)))
	...
	if agentName != "" || agentExact {
		q += ` AND TRIM(LEADING '@' FROM t.agent_name) = ?`
...
// resolved Entity, no embedded message list (F1/F9) — the Go service calls
...
func (s *Store) AgentAttributionCandidates(ctx context.Context, sessionID, agentName string) ([]store.EntityRef, error) {
	...
	return s.queryEntityRefs(ctx, q, sessionID, agentName)
}
...
// outcome (sessionStrategicOutcome's decision logic).
func (s *Store) SessionAttributionEntities(ctx context.Context, sessionID string) ([]store.EntityRef, error) {
	const q = `
		...
		WHERE ua.method <> 'unallocated'
		  AND t.session_id = ?
		  AND ua.entity_type <> ''`
	return s.queryEntityRefs(ctx, q, sessionID)
}

func (s *Store) queryEntityRefs(ctx context.Context, q string, args ...any) ([]store.EntityRef, error) {
	rows, err := s.db.QueryContext(ctx, q, args...)
	if err != nil {
		return nil, err
	}
...
// DirectiveSessions implements store.RecoveryStore. Entity IS resolved here —
// legitimately, a straight MIN() column off the brief_directive interval with
// no decision logic.
...
// parameter (see store.go's doc comment) — added here as the only signature
// that can express "remote" relative to the hub.
...
// overlap-seconds ranking (F1) — the Go service computes overlap from
// Start/End and picks the winner (mirrors synthesize_remote.go:239, already
// done in Go today).
...
// placeholderList returns "?,?,...,?" with n placeholders.

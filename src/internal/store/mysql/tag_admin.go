...
// bindings via the schema's foreign key. Returns the number of value rows
...
// of key (the CLI's key-level `tags describe`, distinct from
// UpdateTagValueDescription's per-value write). Not-found correctness
// mirrors UpdateTagValueDescription: MySQL's RowsAffected counts CHANGED
// rows, so writing back the same description is a 0-row no-op
// indistinguishable from a missing key without an existence check.
...
// SeedTags bulk-defines every spec via DefineTag — the wizard/setup
// first-run universal-key seeding path.
